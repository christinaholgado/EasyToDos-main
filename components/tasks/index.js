...
const Tasks = (props) => {
  ...
  const renderItem = ({ item, index }) => (
    //item is what you are fetching from the key
    //Instead of just showing each object (each task), load all of that into the task component
    <Task item={item} />
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
          ...
          //^when the list is empty, EmptyTaskList component
          //If the list was empty (taskData is null), show the picture in the EmptyTaskList component
          style={{width: '100%'}}
