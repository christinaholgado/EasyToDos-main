      ...
      {!editMode ? (
        <>
          <Text style={props.item.completed ? styles.completedTaskText : styles.taskText}>
            {props.item.completed ? props.item.task + ' was completed at ' + dateText : props.item.task} 
          </Text>
          <TouchableOpacity style={styles.smallIcon} onPress={() => doEdit()}>
            <Icon name="edit" size={18} color={props.item.completed ? "#FFFFFF" : "#8B8B8B"} />
          </TouchableOpacity>
