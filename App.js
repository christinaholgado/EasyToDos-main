...
const App: () => React$Node = () => {
  ...
  useEffect(() => {
    ...
    //^this line is a listener? --> when something changes in authentication
    //^this line is only run when there is a change in authentication
    return subscriber;
  }, []);
  
  const onAuthStateChanged = user => {
    setUser(user);
    //setUser is the setter for state variable user
    if ((user) && (currentScreen === 'login')) {
      setCurrentScreen('tasks');
    }
    if (initializing) {
      setInitializing(false);
      //sets initializing to "false" when you log in or you want to show the user the page for log in 
    }
  };
  ...
  if (initializing) {
    //When the app runs for the first time when we render the application the line below would run.
    //If initializing is set to true, it will jump out of the function and return null 
    //need to switch off the initializing --> make initializing false
    return null;
  }
